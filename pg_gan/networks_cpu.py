...
def get_weight(shape, gain=np.sqrt(2), use_wscale=False, fan_in=None):
    ...
    if use_wscale:
        ...
        return tf.Variable('weight', shape=shape, initializer=tf.initializers.random_normal(0, std))
...
#    b = tf.get_variable('bias', shape=[x.shape[1]], initializer=tf.initializers.zeros())
    b = tf.Variable('bias', shape=[x.shape[-1]], initializer=tf.initializers.zeros())
    b = tf.cast(b, x.dtype)
    if len(x.shape) == 2:
        return x + b
    else:
